...
public class Magazine extends BibliographicPtoducts {
     ...
     *                            converted in the constructor to an enum value)
     */
    public Magazine(String name, int pageNumber, GregorianCalendar publicationDate, double price, String url,
            int frequencyOfIssuance, int category) {
    ...
    @Override
    /**
     * Method that generates a random ID for the magazine that is a unique 3
     * alphanumeric digit identifier (A-Z, 0-9)
     * 
    ...
    /**
     * Method that returns the magazine's category flag value
     * @return getCategory Magazine's category flag value
     */
    @Override
    public int getGenreOrCategoryByFlag() {
        int getCategory = 0;
        if (this.category == Category.VARIETY) {
            getCategory = 1;
        } else if (this.category == Category.DESIGN) {
            getCategory = 2;
        }
        else if (this.category == Category.SCIENCE) {
            getCategory = 3;
        }
